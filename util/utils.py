...
def evaluate_batch(batch_size, gt_batch, pred_batch, mask_batch):
    gt_batch = torch.from_numpy(gt_batch).type(torch.FloatTensor).cuda()
    pred_batch = torch.from_numpy(pred_batch).type(torch.FloatTensor).cuda()
    mask_batch = torch.from_numpy(mask_batch).type(torch.FloatTensor).cuda()
